...
def main():
    ...
    missing_c1.sort(key=lambda x: x["word"].lower())
    print(f"Total missing C1 words: {len(missing_c1)}")

    if not missing_c1:
        print("No missing C1 words left!")
        return

    # 4. Select the first 256 words (16 grids)
    target_count = min(256, len(missing_c1))
    selected_words = missing_c1[:target_count]
    print(f"Selected {target_count} C1 words for this batch.")

...
namespace Englishtest
{
    public partial class GuideReadingWindow : Window
    {
        ...
        private bool _showChinese = false;
        private static readonly string VoiceConfigPath = System.IO.Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "voice_config.txt");
        ...
        private void ConfigureVoice()
        {
            try
            {
                ...
                if (voices.Any())
                {
                    ...
                    // Default selection: find Zira, first en-US, or saved voice setting
                    var defaultIndex = -1;
                    if (System.IO.File.Exists(VoiceConfigPath))
                    {
                        try
                        {
                            var savedVoice = System.IO.File.ReadAllText(VoiceConfigPath).Trim();
                            if (!string.IsNullOrEmpty(savedVoice))
                            {
                                defaultIndex = voices.FindIndex(v => v.VoiceInfo.Name == savedVoice);
                            }
                        }
                        catch { }
                    }
                    ...
                    if (defaultIndex < 0)
                    {
                        defaultIndex = voices.FindIndex(v => v.VoiceInfo.Name.Contains("Zira"));
                    }
        ...
        private void VoiceComboBox_SelectionChanged(object sender, SelectionChangedEventArgs e)
        {
            if (VoiceComboBox.SelectedValue is string voiceName)
            {
                try
                {
                    ...
                    System.IO.File.WriteAllText(VoiceConfigPath, voiceName);
                }
                catch { }
            }
        }
        ...
        private void SpeakCurrentSegment()
        {
            ...
            var cleanText = System.Text.RegularExpressions.Regex.Replace(text, @"^[A-Za-z]{3,10}:\s*", "");
        ...
        private void LoadDefaultButton_Click(object sender, RoutedEventArgs e)
        {
            ...
            if (_content.LoadFromDefaultPath())
            {
                ...
                LoadFriendsButton.Content = "🎬 隨機六人行對話";
                UpdateChineseToggle();
                UpdateProgressText();
            }
            else
